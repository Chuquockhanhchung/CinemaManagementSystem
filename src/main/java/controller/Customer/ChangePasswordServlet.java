...
public class ChangePasswordServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            dao.resetPassword(c.getEmail(), newPassword);

            // Update the password in the session
            c.setPass(s.encode(newPassword));
            session.setAttribute("user", c);

            session.setAttribute("succMess", "Thay đổi mật khẩu thành công");
            response.sendRedirect("user_information.jsp" + "?CustomerID=" + c.getIdCustomer());
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("failMess", "Có lỗi xảy ra: " + e.getMessage());
            response.sendRedirect("user_information.jsp" + "?CustomerID=" + c.getIdCustomer());
        }
    }
