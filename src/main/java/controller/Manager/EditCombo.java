 ...
 * @author Chi
 */
public class EditCombo extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ComboDAO daoc = new ComboDAO(DBContext.getConn());
            int id = Integer.parseInt(request.getParameter("id"));
            Combo combo = daoc.getComboByID(id);
            ArrayList<Product> list =dao.getAllProduct();
            ArrayList<String> idp = daoc.getProduct(id);
            HttpSession session = request.getSession();
            session.setAttribute("product", list);
            session.setAttribute("combo", combo);
            request.setAttribute("id", idp);
            request.getRequestDispatcher("manager/CMS/edit_combo.jsp").forward(request, response);
        ...
        try {
            ...
            if(name.trim() == "" || des.trim() == ""  || img.trim() == "" ||  request.getParameter("price").trim() == ""){
                ...
                if(type.length == 0 ){
                    ...
                    dao.updateCombo(c);
                    dao.deleteCombohasProduct(id);
                    for (String t : type) {
                        dao.addCombohasProduct(id,Integer.parseInt(t));
                    }
