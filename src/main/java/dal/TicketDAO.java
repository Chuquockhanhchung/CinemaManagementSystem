...
public class TicketDAO extends DBContext{
    ...
    public Movie getMovieByShowTime(int id) throws SQLException {
        try {
            String sql = "select m.MovieID,m.MovieName,m.Description,m.Types,m.Image,m.Actors,m.Status,m.Duration,m.Price,m.ReleaseDate from showtime s join movie_all m on s.MovieID =m.MovieID where s.ShowtimeID=?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
